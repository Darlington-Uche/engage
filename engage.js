...
bot.command('loc', async (ctx) => {
  ...
  // Toggle lock state
  groupData.groupLocked = !groupData.groupLocked;
  
  await ctx.telegram.setChatPermissions(ctx.chat.id, {
    can_send_messages: !groupData.groupLocked,
    can_send_media_messages: !groupData.groupLocked,
    can_send_other_messages: !groupData.groupLocked,
    can_add_web_page_previews: !groupData.groupLocked
  });

  await saveGroupData(groupId, groupData);
  
  const status = groupData.groupLocked ? '🔒 LOCKED' : '🔓 UNLOCKED';
  await ctx.reply(`Group has been ${status}\n\nOnly admins can send messages when locked.`);
});
...
// Message handler for slot phase - UPDATED WITH NEW RULES
bot.on('message', async (ctx) => {
  ...
  if (groupData.groupLocked && !await isAdmin(ctx, userId)) {
    await ctx.deleteMessage();
    return;
  }
  ...
  if (groupData.state === BOT_STATES.SLOT_OPEN) {
    ...
    // User already dropped a link - check for multiple links (RULE 3)
    ...
    if (isXLink(messageText)) {
      ...
      // Check if X username is in muted list (RULE 2)
      ...
      // Check for duplicate X usernames across users (RULE 1)
      let duplicateFound = false;
      let duplicateUserId = null;
      
      for (const [existingUserId, existingUserData] of groupData.userLinks.entries()) {
        if (existingUserData.xUsername.toLowerCase() === xUsername.toLowerCase()) {
          duplicateFound = true;
          duplicateUserId = existingUserId;
          break;
        }
      }

      if (duplicateFound) {
        // Delete both links and mute both users for 30 mins
        await ctx.deleteMessage();
        
        // Mute current user
        await muteUser(ctx, groupData, userId, null, 30);
        
        // Mute the duplicate user
        await muteUser(ctx, groupData, duplicateUserId, null, 30);
        
        // Remove both links
        groupData.userLinks.delete(userId);
        groupData.userLinks.delete(duplicateUserId);
        
        await ctx.reply(`🔇 Both users muted for 30 minutes - duplicate X username detected: ${xUsername}`);
        await saveGroupData(groupId, groupData);
        return;
      }

      // If all checks pass, save the link
    ...
    // During SR phase, only allow media messages
    ...
    if (hasMedia) {
      ...
      if (isInSRList) {
        ...
        // UPDATED: Reply with user's X username
        await ctx.reply(`×:${xUsername} Your SR has been received ✅`);
        await saveGroupData(groupId, groupData);
      }
    }
  }
